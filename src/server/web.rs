...
//         if let Some(pr) = x {
//             stream.write_all(pr.unwrap().as_slice()).await.unwrap();
//             // if pr.req_id == "-1" {
//             //     break;
//             // }
//         } else {
...
// }
//
// async fn stream_handler(stream: &mut TcpStream, otx: mpsc::Sender<Result<Vec<u8>, Error>>) -> anyhow::Result<()> {
...
//                 }
//
//                 let warp_req: R = R { body_tx: otx.clone(), req: req, header_tx: () };
//                 // 获取发送Request的Sender
